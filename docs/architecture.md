...
sequenceDiagram
  ...
  M->>F: Create checkout session using RequestBuilder for payload and Guzzle for HTTP
...
```mermaid
flowchart LR
  A[Webhook HTTP Request] --> B[WebhookRequestValidator - signature check]
  B --> C[WebhookRequestParser - build WebhookEvent]
  C --> D[WebhookIdempotencyService - ensure once-only]
  D --> E[WebhookRouter - event type mapping]
  E --> F[Concrete Handler - Refund/PaymentIntent/CheckoutSession]
  F --> G[TransactionService - create or update transaction]
  F --> H[Magento Order/Invoice - update state]
...
- Checkout Create controller depends on `Api\\Checkout\\RequestBuilderInterface` to build the checkout session payload (default: `Service/Checkout/RequestBuilder` + `Service/Checkout/MetadataBuilder`).
